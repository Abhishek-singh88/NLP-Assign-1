...
function wordToSymbols(word) {
  // split into characters; append end-of-word marker
...
function trainBPEOnce(words, maxVocabSize, maxMerges) {
...
function countOOV(words, vocab) {
  let oov = 0;
  for (let i = 0; i < words.length; i += 1) {
    const chars = Array.from(words[i]);
    let ok = true;
    for (let j = 0; j < chars.length; j += 1) {
      if (!vocab.has(chars[j])) {
        ok = false;
        break;
      }
    }
...
function tokenizeWords(words, merges) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    const tokens = applyBPE(words[i], merges);
    // drop end-of-word marker from counts
...
function main() {
  ...
  const outDir = process.argv[3] || path.join("outputs", "bpe");
  ...
  const t0 = nowMs();
  const model = trainBPEOnce(trainWords, maxTarget, maxMergesArg);
  ...
  for (let i = 0; i < vocabSizes.length; i += 1) {
    ...
    const oovTrain = countOOV(trainWords, model.baseVocab);
    const oovTest = countOOV(testWords, model.baseVocab);
    ...
    const modelPrefix = path.join(outDir, "bpe_" + vocabSize);
    ...
    const segExamples = sampleWords.map((w) => {
      const toks = applyBPE(w, merges).filter((t) => t !== "</w>");
      return w + "\t" + toks.join(" ");
    });
    ...
    metrics.push({
      ...
      oov_rate_train: trainWords.length ? oovTrain / trainWords.length : 0,
      oov_rate_test: testWords.length ? oovTest / testWords.length : 0,
  ...
  const csvLines = [
    "vocab_size,target_vocab_size,train_words,test_words,avg_tokens_per_word_train,avg_tokens_per_word_test,oov_rate_train,oov_rate_test,compression_ratio,train_time_ms,tokenization_time_ms,memory_bytes_est"
  ];
  ...
  console.log("BPE training done. Outputs in:", outDir);
}
